...
import { formatNaira, todayStr, monthStartStr, daysAgoStr } from "../utils";
...
export default function Reports({ user }: Props) {
  ...
  const computeRange = useCallback(() => {
    ...
    if (range === "week") {
      return { f: daysAgoStr(6), t: today };
    }
    if (range === "month") {
      return { f: monthStartStr(), t: today };
    }
    return { f: from, t: to };
  }, [range, from, to]);
  ...
  const load = useCallback(async () => {
    ...
    const [txnRes, expRes, prodRes, restockRes, priceRes] = await Promise.all([
      supabase
        ...
        .select("id, sale_date, total_amount, is_voided, sales(product_id, quantity_units, total_price, cost_price)")
        .eq("is_voided", false)
        .gte("sale_date", f)
        .lte("sale_date", t)
        .order("sale_date"),
    ...
    for (const txn of txns) {
      ...
      for (const item of txn.sales || []) {
        ...
        ds.cost += Number(item.quantity_units) * costPerUnit;
      }
    }
