...
import { formatNaira, todayStr, formatMonthYear, monthStartStr } from "../utils";
  ...
  async function load() {
    ...
    for (const txn of txns) {
      ...
      for (const item of items) {
        ...
        // Use the buy price that was actually recorded on this sale at the
        // time it happened. Falls back to today's product price only for
        // older rows recorded before cost_price started being captured —
        // new sales are always exact, never retroactively wrong if a price
        // changes later in the month.
        const costPerUnit = item.cost_price ?? p?.buy_price ?? 0;
        monthCost += Number(item.quantity_units) * costPerUnit;
